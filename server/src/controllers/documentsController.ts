import type { Request, Response } from "express";
import type { Silo } from "../services/prisma.js";
import { documentService } from "../services/documentService.js";

const toSilo = (value: string): Silo => value as Silo;

export const uploadDocument = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const file = req.file;
  const appId = req.params.appId;

  if (!file) return res.status(400).json({ error: "No file uploaded" });

  const doc = await documentService.upload(user, silo, appId, file);
  return res.status(201).json(doc);
};

export const getDocument = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const id = req.params.id;

  const doc = await documentService.get(user, silo, id);
  if (!doc) return res.status(404).json({ error: "Not found" });

  return res.json(doc);
};

export const previewDocument = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const id = req.params.id;

  const file = await documentService.download(user, silo, id);
  if (!file) return res.status(404).json({ error: "Not found" });

  res.setHeader("Content-Type", file.mimeType);
  return res.send(file.buffer);
};

export const downloadDocumentHandler = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const id = req.params.id;

  const result = await documentService.download(user, silo, id);
  if (!result) return res.status(404).json({ error: "Not found" });

  res.setHeader("Content-Type", result.mimeType);
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${result.name}"`
  );
  return res.send(result.buffer);
};

export const acceptDocumentHandler = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const id = req.params.id;

  const result = await documentService.accept(user, silo, id, user.id);
  if (!result) return res.status(404).json({ error: "Not found" });

  return res.json(result);
};

export const rejectDocumentHandler = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const id = req.params.id;

  const result = await documentService.reject(user, silo, id, user.id);
  if (!result) return res.status(404).json({ error: "Not found" });

  return res.json(result);
};

export const downloadAllDocumentsHandler = async (req: Request, res: Response) => {
  const user = req.user!;
  const silo = toSilo(req.params.silo);
  const appId = req.params.appId;

  const result = await documentService.downloadAll(user, silo, appId);
  if (!result) return res.status(404).json({ error: "Not found" });

  res.setHeader("Content-Type", "application/zip");
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${result.fileName}"`
  );

  return res.send(result.zipBuffer);
};
